...
getcontext().prec = 4 #precision de 7 decimales
getcontext().rounding=ROUND_HALF_DOWN #redondeo hacia abajo

UPDATE_TIME = 60 #tiempo de actualizacion del estado en la bd en segundos
...
class CreateCert(Thread):
    ...
    def __init__(self, w, simbolico, minuto, tiempo):
        ...
        self.loop = Event()
        self.doit = Event() #evento para crear el titulo
        self.doit.set() #inicializa en set el evento
        self.watcher = w #instancia de WatchAndPost
        self.api = WebApi()
...
class SerialCom(Thread):
    ...
    def run(self):
        ...
        '''
        while not self.loop.is_set():
            self.serial.receive_data() #revisa constantemente el puerto serial
            
            #recibe la orden de crear el certificado
            if self.serial.orderCertificate == True:
                cert = self.ca.create() #crea el certificado
                logging.debug('cert: %s', cert) 
                self.serial.open_certificate(cert) #lo envia a serial_io
                self.serial.orderCertificate = False #cambia el flag
...
def main():
    ...
    TIEMPO_REALIZACION_TITULO = 5 #minutos
